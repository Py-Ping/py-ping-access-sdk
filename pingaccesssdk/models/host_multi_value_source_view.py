...
class HostMultiValueSourceView(Model):
    """Configuration for the host source.
    ...
    def __init__(self, headerNameList: list, listValueLocation: ListValueLocation) -> None:
    ...
    def __eq__(self, other) -> bool:
        if isinstance(other, HostMultiValueSourceView):
            return self.__dict__ == other.__dict__
        return NotImplemented

    def __hash__(self) -> int:
        return hash(frozenset([self.headerNameList, self.listValueLocation]))

    @classmethod
    def from_dict(cls, python_dict: dict):
        valid_data = {}
        for k, v in python_dict.items():
            if k in ["headerNameList", "listValueLocation"]:
                if k == "headerNameList":
