...
class ChainCertificateView(Model):
    """A chain certificate.

    Attributes
    ----------
    id: int
        The Id for the chain certificate.

    alias: str
        (sortable) The Alias for the chain certificate.

    expires: int
        (sortable) The date at which the chain certificate expires as the number of milliseconds since January 1, 1970, 00:00:00 GMT.

    issuerDn: str
        (sortable) The Issuer DN for the chain certificate.

    md5sum: str
        The MD5 checksum of the chain certificate. The value will be set to "" when in FIPS mode.

    serialNumber: str
        (sortable) The Serial Number for the chain certificate.

    sha1sum: str
        The SHA1 checksum of the chain certificate.

    sha256sum: str
        The SHA256 checksum of the chain certificate.

    signatureAlgorithm: str
        (sortable) The Signature Algorithm used by the chain certificate.

    status: CertStatus
        A high-level status for the chain certificate.
    ...
    subjectDn: str
        (sortable) The Subject DN for the chain certificate.

    validFrom: int
        (sortable) The date at which the chain certificate is valid from as the number of milliseconds since January 1, 1970, 00:00:00 GMT.
    ...
    def __eq__(self, other) -> bool:
        if isinstance(other, ChainCertificateView):
